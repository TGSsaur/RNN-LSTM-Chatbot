# -*- coding: utf-8 -*-
"""
Created on Mon Mar 16 10:22:18 2020
...
#creating dictionaries that maps each line and id
id2line = {}
for line in lines:
    _line = line.split(' +++$+++ ')
    if len(_line) == 5:
...
#Creating list of all the conversation
conversation_ids = []
for conversation in conversations[:-1]:
    _conversation = conversation.split(' +++$+++ ')[-1][1:-1].replace("'","").replace(" ","")
    conversation_ids.append(_conversation.split(','))
    
 # getting separately the question and answer
questions = []
answers = []
...
#Creating a dictionary that maps each word with its number of occurances
word2count = {}
...
# Creating two dictionaries that maps the questions words and the answers word to a unique integer
threshold = 20
questionswords2int = {}
word_number = 0
...
# Translating all the questions and the answers into integerd
...
# Sorting quesions and answers by the length of questions
...
# Creating placeholder for the input and the target
def model_input():
...
# Getting up the Loss Error,the Optimizer and Gradient Clipping
